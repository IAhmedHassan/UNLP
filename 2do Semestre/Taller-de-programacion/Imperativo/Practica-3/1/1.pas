...
De cada socio se debe almacenar numero de socio, nombre y edad. La carga finaliza con el 
numero de socio 0 y el arbol debe quedar ordenado por numero de socio. La informacion de 
...
procedure InformarExistenciaNombreSocio(a : arbol);
  ...
  function existeSocio(a: arbol; n : cadena15): boolean;
  begin
    if (a <> nil) then begin
      if a^.dato.nombre = n then existeSocio := TRUE;
      else existeSocio := existeSocio(a^.HI) or existeSocio(a^.HD);
    end
...
begin
  writeln;
  writeln('----- Comprobar existencia de socio ----->');
  writeln;
  write('ingrese un nombre: '); readln(n);
  writeln;
  if existeSocio(a, n) 
    then writeln('El socio existe.')
